...
coarse_map = pd.read_csv('../data/highway_map.csv', sep='\s+',
                         names=['x', 'y', 's', 'dx', 'dy'])
fine_map = pd.read_csv('../data/map_fine.dat', sep='\s+')
...
sx = fine_map["sx"]
sy = fine_map["sy"]
theta = fine_map["theta"]

fig, ax = plt.subplots(figsize=(10, 10), dpi=60)

ax.plot(coarse_map["x"], coarse_map["y"], 'o', ms=4,
        label='coarse map waypoints')
ax.plot(x, y, 'r', label='center line splines', zorder=0)
ax.set_xlim(0, 2500)
